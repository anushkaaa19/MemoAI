  ...
  return (
    <header className='sticky top-0 z-40 w-full h-16 bg-white/80 backdrop-blur-xl border-b border-slate-200/60'>
      <div className='flex items-center justify-between h-full px-6'>
        <button onClick={toggleSidebar} className='md:hidden inline flex items-center justify-center w-10 h-10 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-all duration-200' aria-label="Toggle Sidebar">
          <Menu size={24}/>
        </button>
        <div className='hidden md:block'></div>
        <div className='flex items-center gap-3'>
          {/* Notification Bell with Component */}
          <NotificationCenter />
